<div class="" x-on:pedido-updated.window="$wire.$refresh()">
    ...
    <form enctype="multipart/form-data" method="POST" action="{{ route('documento.store-pedido', $pedido) }}">
        ...
        <x-modal.card wire:model='modalDocumento' title="Adicionar Documento">
            <div x-data="{
                ...
                <x-select :async-data="route('tipo-documento.search')" x-model.defer="tipoDocumento" label="Tipo de Documento" option-value="id"
                    option-label="nome" name="tipo_documento_id">
    ...
    @if ($modalAcesso)
        <x-modal.card wire:model='modalAcesso'>
            ...
            <x-slot name="footer">
                <x-button color="primary" label="Atualizar" wire:click='saveAcessos({{ $modalAcesso }})' />
                <x-button label="Cancelar" x-on:click="close" />
            </x-slot>
        </x-modal.card>
    @endif
